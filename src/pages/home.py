...
card_GreenSpaceHack_Number_of_questionnaires_Feuersee = dbc.Card(
    dbc.CardBody(
        [
            html.H4([html.I(className="bi bi-question-square"), " Anzahl Fragebögen Feuersee"], className="text-nowrap"),
...
card_average_nest_score_feuersee = dbc.Card(
    dbc.CardBody(
        [
            html.H4([html.I(className="bi bi-bar-chart"), " Durchschnittlicher NEST Score Feuersee"], className="text-nowrap"),
...
card_inaturalist_observations = dbc.Card(
    dbc.CardBody(
        [
            html.H4([html.I(className="bi bi-eye"), " Anzahl Beobachtungen iNaturalist"], className="text-nowrap"),
...
card_inaturalist_species = dbc.Card(
    dbc.CardBody(
        [
            html.H4([html.I(className="bi bi-list-task"), " Unterschiedliche Arten iNaturalist"], className="text-nowrap"),
...
card_inaturalist_invasive_species = dbc.Card(
    dbc.CardBody(
        [
            html.H4([html.I(className="bi bi-exclamation-triangle"), " Invasive Arten iNaturalist"], className="text-nowrap"),
...
card_inaturalist_threatened_species = dbc.Card(
    dbc.CardBody(
        [
            html.H4([html.I(className="bi bi-shield-exclamation"), " Bedrohte Arten iNaturalist"], className="text-nowrap"),
...
def update_observations_distribution_graph(selected_metric):
    if selected_metric == 'observations':
        df_observation_filtered_df = df_eyeonwater.groupby('month_year')['n_code'].count().reset_index()
        df_observation_filtered_df.sort_values(by='month_year')
        fig = px.bar(df_observation_filtered_df, x='month_year', y='n_code', labels={'month_year':'Zeit', 'n_code': 'Beobachtungen'})
